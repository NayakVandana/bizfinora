...
/**
 * Redirect guests to login when no API token is present.
 */
export function requireAuthPage(returnTo?: string): boolean {
    if (getUserApiToken()) {
        return true;
    }

    router.visit(loginUrl(returnTo ?? window.location.pathname));
